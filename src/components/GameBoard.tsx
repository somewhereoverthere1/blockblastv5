...
export const GameBoard: React.FC<GameBoardProps> = ({ grid, selectedShape, onCellClick }) => {
  ...
  useEffect(() => {
    if (hoverPosition && selectedShape) {
      ...
      const centerRowOffset = (minRow + maxRow) / 2;
      const centerColOffset = (minCol + maxCol) / 2;
      
      // Add preview of the shape, centered on the hover position
      let canPlace = true;
      
      selectedShape.blocks.forEach(([r, c]) => {
        // Use Math.round to ensure we get the closest grid cell to the true center
        const newRow = Math.round(hoverRow + r - centerRowOffset);
        const newCol = Math.round(hoverCol + c - centerColOffset);
              ...
              <div
                ...
                onClick={() => {
                  if (selectedShape) {
                    ...
                    // Use Math.round to ensure we get the closest grid cell to the true center
                    onCellClick(Math.round(rowIndex - centerRowOffset), Math.round(colIndex - centerColOffset));
                  } else {
